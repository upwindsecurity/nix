...
in
{
    ...
    dev = let
      devOptions = {
        ...
        authAudience = mkOption {
          type = types.str;
          description = "Override authentication audience.";
        };
      };
    in
      mkOption {
        type = with types; submodule devOptions;
        description = "Override options to enable dev/test of unreleased versions.";
        default = null;
      };
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."upwind/agent.yaml".text = ''
      ...
      region: "${cfg.region}"
      api-host: "https://agent${if cfg.region == "eu" then ".eu" else ""}.${domain}"
      auth-endpoint: "https://oauth${if cfg.region == "eu" then ".eu" else ""}.${domain}/oauth/token"
